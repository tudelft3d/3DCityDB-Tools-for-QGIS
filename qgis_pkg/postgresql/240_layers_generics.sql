...
--
--
-- CREATE LAYERS FOR MODULE GENERICS
--
--
-- ****************************************************************************
-- ****************************************************************************

--SELECT qgis_pkg.drop_layers(usr_schema:= 'qgis_user', cdb_schema:= 'citydb', feat_type := 'Generics'); 
--DELETE FROM qgis_user.layer_metadata WHERE cdb_schema = 'citydb' AND feature_type = 'Generics';

----------------------------------------------------------------
-- Create FUNCTION QGIS_PKG.GENERATE_SQL_LAYERS_GENERIC_CITYOBJECT
----------------------------------------------------------------
DROP FUNCTION IF EXISTS    qgis_pkg.generate_sql_layers_generics(varchar, varchar, integer, integer, numeric, geometry, boolean) CASCADE;
CREATE OR REPLACE FUNCTION qgis_pkg.generate_sql_layers_generics(
cdb_schema 			varchar,
usr_name            varchar,
...
FROM (
	...
	FROM 
		',cdb_schema,'.generic_cityobject AS o
...
FROM
	...
	INNER JOIN ',cdb_schema,'.cityobject AS co ON (g.co_id = co.id AND co.objectclass_id = ',r.class_id,')
  	INNER JOIN ',cdb_schema,'.generic_cityobject AS o ON (o.id = co.id AND o.objectclass_id = ',r.class_id,');
...
EXCEPTION
	WHEN QUERY_CANCELED THEN
		RAISE EXCEPTION 'qgis_pkg.generate_sql_layers_generics(): Error QUERY_CANCELED';
	WHEN OTHERS THEN
		RAISE EXCEPTION 'qgis_pkg.generate_sql_layers_generics(): %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION qgis_pkg.generate_sql_layers_generics
(varchar, varchar, integer, integer, numeric, geometry, boolean)
 IS 'Create layers for module Generics';

----------------------------------------------------------------
-- Create FUNCTION QGIS_PKG.CREATE_LAYERS_GENERICS
----------------------------------------------------------------
DROP FUNCTION IF EXISTS    qgis_pkg.create_layers_generics(varchar, varchar, integer, integer, numeric, geometry, boolean) CASCADE;
...
BEGIN
SELECT qgis_pkg.generate_sql_layers_generics(
...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION qgis_pkg.create_layers_generics
	(varchar, varchar, integer, integer, numeric, geometry, boolean)
	IS 'Create layers for module Generics';

SELECT qgis_pkg.create_layers_generics(cdb_schema := 'citydb3', force_layer_creation := FALSE);
...
BEGIN
SELECT qgis_pkg.generate_sql_layers_generics() INTO sql_statement;
