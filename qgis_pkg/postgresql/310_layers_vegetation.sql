...
--
--
-- CREATE LAYERS FOR MODULE VEGETATION
--
--
-- ****************************************************************************
-- ****************************************************************************

--SELECT qgis_pkg.drop_layers(usr_schema:= 'qgis_user', cdb_schema:= 'citydb', feat_type := 'Vegetation'); 
--DELETE FROM qgis_user.layer_metadata WHERE cdb_schema = 'citydb' AND feature_type = 'Vegetation';
...
feature_type 	varchar := 'Vegetation';
srid_id         integer; 
num_features    bigint;
...
FROM (
	...
	FROM 
		',cdb_schema,'.solitary_vegetat_object AS o
...
CREATE MATERIALIZED VIEW         ',usr_schema,'.',mview_name,' AS
	...
	FROM
		',cdb_schema,'.plant_cover AS o
		INNER JOIN ',cdb_schema,'.cityobject AS co ON (o.id = co.id AND o.objectclass_id = ',r.class_id,' ',sql_where,')
		INNER JOIN ',cdb_schema,'.surface_geometry AS sg ON (sg.root_id = o.',t.lodx_label,'_multi_solid_id AND sg.geometry IS NOT NULL)
	WHERE
		o.',t.lodx_label,'_multi_solid_id IS NOT NULL 
	GROUP BY sg.cityobject_id
	UNION
...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION qgis_pkg.generate_sql_layers_vegetation(varchar, varchar, integer, integer, numeric, geometry, boolean) IS 'Create layers for module LandUse';
