...
CASE 
	WHEN cdb_schema IS NULL AND mview_name IS NULL THEN -- refresh all existing materialized views
	RAISE NOTICE 'Refreshing all materialized views in schema %', qgis_schema;
		FOR r IN 
			...
			WHERE pg_class.relkind = 'm' AND pg_namespace.nspname = qgis_schema
			ORDER BY mview_name
		LOOP
			start_timestamp := clock_timestamp();
			EXECUTE format('REFRESH MATERIALIZED VIEW %I.%I', qgis_schema, r.mview_name);
			stop_timestamp := clock_timestamp();
			EXECUTE format('SELECT count(co_id) FROM %I.%I', qgis_schema, r.mview_name) INTO mv_n_features;

			UPDATE qgis_pkg.layer_metadata AS lm SET
				n_features    = mv_n_features,
				refresh_date  = stop_timestamp
			WHERE lm.mv_name = r.mview_name;
			RAISE NOTICE 'Refreshed materialized view "%.%" in %', qgis_schema, r.mview_name, stop_timestamp-start_timestamp; 
		END LOOP;
		f_stop_timestamp := clock_timestamp();		
		RAISE NOTICE 'All materialized views in schema "%" refreshed in %!', qgis_schema, f_stop_timestamp-f_start_timestamp; 	
		RETURN 1;

	WHEN cdb_schema IS NOT NULL THEN -- refresh all existing materialized views for that schema
		IF EXISTS (
			--SELECT 1 FROM pg_catalog.pg_namespace WHERE pg_namespace.nspname=cdb_schema
			SELECT 1
			FROM pg_catalog.pg_class
				INNER JOIN pg_catalog.pg_namespace ON pg_class.relnamespace = pg_namespace.oid
			WHERE pg_class.relkind = 'm' AND pg_namespace.nspname = qgis_schema
				AND split_part(pg_class.relname::text, '_', 3) = cdb_schema
				--AND pg_class.relname LIKE '_g_'||cdb_schema||'_%'
			LIMIT 1
			) THEN
			RAISE NOTICE 'Refreshing all materialized views associated to schema "%"', cdb_schema;		
			FOR r IN 
				SELECT pg_namespace.nspname AS table_schema, pg_class.relname AS mview_name
				FROM pg_catalog.pg_class
					INNER JOIN pg_catalog.pg_namespace ON pg_class.relnamespace = pg_namespace.oid
			...
			END LOOP;
			f_stop_timestamp := clock_timestamp();		
			RAISE NOTICE 'All materialized views for citydb schema "%" refreshed in %!!', cdb_schema, f_stop_timestamp-f_start_timestamp; 	
			RETURN 1;
		ELSE
	...
	WHEN mview_name IS NOT NULL THEN -- refresh only a specific materialized views
		IF EXISTS (SELECT 1 
			...
			RAISE NOTICE 'Refreshing materialized view "%.%""', qgis_schema, mview_name;
			start_timestamp := clock_timestamp();
			EXECUTE format('REFRESH MATERIALIZED VIEW %I.%I', qgis_schema, mview_name);
			stop_timestamp := clock_timestamp();

			EXECUTE format('SELECT count(co_id) FROM %I.%I', qgis_schema, mview_name) INTO mv_n_features;
			...
			RAISE NOTICE 'Refreshed materialized view "%.%" in %', qgis_schema, mview_name, stop_timestamp-start_timestamp; 
			RETURN 1;
		ELSE
...
--PERFORM qgis_pkg.refresh_mview(cdb_schema := 'citydb');
