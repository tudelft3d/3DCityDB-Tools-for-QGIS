...
-- create all layers of CityGML module "Generics". 
--
-- ***********************************************************************
...
codelist_cols_array	varchar[][] := ARRAY[['generic_cityobject','class'],['generic_cityobject','function'],['generic_cityobject','usage']];
...
root_class := 'GenericCityObject';
---------------------------------------------------------------
-- Create LAYER GENERICS_LOD0-4
---------------------------------------------------------------
FOR r IN 
	SELECT * FROM (VALUES
	('GenericCityObject'::varchar, qgis_pkg.class_name_to_class_id(cdb_schema, 'GenericCityObject', NULL)::integer, 'gen_cityobj'::varchar)
	) AS t(class_name, class_id, class_label)
LOOP

---------------------------------------------------------------
-- Create LAYER GENERIC_CITYOBJECT_LOD0-4 TerrainIntersectionCurve
---------------------------------------------------------------
	FOR t IN 
		SELECT * FROM (VALUES
...
sql_feat_count := concat('
	...
	FROM 
		',qi_cdb_schema,'.generic_cityobject AS o
...
--av_name			:= concat('_a_',cdb_schema,'_gen_cityobj');
gv_name			:= concat('_g_',l_name);
qml_form_name  := 'gen_form.qml';
...
END IF;
	...
	END LOOP; -- END Loop TIC LoD0-4

---------------------------------------------------------------
-- Create LAYER GENERICS_LOD0-4 (Polygon-based layers)
---------------------------------------------------------------
	FOR t IN 
		SELECT * FROM (VALUES
		('LoD0'::varchar, 'lod0'::varchar),
...
gv_name			:= concat('_g_',l_name);
qml_form_name  := 'gen_form.qml';
qml_symb_name  := 'poly_dark_grey_symb.qml';
qml_3d_name    := 'poly_dark_grey_3d.qml';
trig_f_suffix := 'generic_cityobject';
qi_l_name  := quote_ident(l_name); ql_l_name := quote_literal(l_name);
qi_gv_name  := quote_ident(gv_name); ql_gv_name := quote_literal(gv_name);
--qi_av_name   := quote_ident(av_name); ql_av_name := quote_literal(av_name);
...
FROM
	',qi_usr_schema,'.',qi_gv_name,' AS g 
	INNER JOIN ',qi_cdb_schema,'.cityobject AS co ON (g.co_id = co.id AND co.objectclass_id = ',r.class_id,')
  	INNER JOIN ',qi_cdb_schema,'.generic_cityobject AS o ON (o.id = co.id AND o.objectclass_id = ',r.class_id,');
COMMENT ON VIEW ',qi_usr_schema,'.',qi_l_name,' IS ''View of ',r.class_name,' ',t.lodx_name,' in schema ',qi_cdb_schema,''';
ALTER TABLE ',qi_usr_schema,'.',qi_l_name,' OWNER TO ',qi_usr_name,';
');
