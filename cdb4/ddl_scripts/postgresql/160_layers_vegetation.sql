...
-- create all layers of CityGML module "Vegetation". 
--
-- ***********************************************************************

----------------------------------------------------------------
-- Create FUNCTION QGIS_PKG.GENERATE_SQL_LAYERS_VEGETATION
----------------------------------------------------------------
DROP FUNCTION IF EXISTS    qgis_pkg.generate_sql_layers_vegetation(varchar, varchar, integer, integer, numeric, geometry, boolean) CASCADE;
CREATE OR REPLACE FUNCTION qgis_pkg.generate_sql_layers_vegetation(
usr_name            varchar,
cdb_schema 			varchar,
...
---------------------------------------------------------------
-- Create LAYER SOL_VEG_OBJ_LOD1-4
---------------------------------------------------------------
FOR r IN 
	SELECT * FROM (VALUES
...
codelist_cols_array	:= ARRAY[['solitary_vegetat_object','class'],['solitary_vegetat_object','function'],['solitary_vegetat_object','usage'], ['solitary_vegetat_object','species']];
...
FROM (
	...
	FROM 
		',qi_cdb_schema,'.solitary_vegetat_object AS o
...
--av_name			:= concat('_a_',cdb_schema,'_veg_sol_veg_obj');
gv_name			:= concat('_g_',l_name);
qml_form_name  := 'veg_svo_form.qml';
qml_symb_name  := 'poly_green_symb.qml';
qml_3d_name    := 'poly_green_3d.qml';
trig_f_suffix := 'solitary_vegetat_object';
...
FROM
	...
	INNER JOIN ',qi_cdb_schema,'.cityobject AS co ON (g.co_id = co.id AND co.objectclass_id = ',r.class_id,')
  	INNER JOIN ',qi_cdb_schema,'.solitary_vegetat_object AS o ON (o.id = co.id AND o.objectclass_id = ',r.class_id,');
...
END IF;
	...
	END LOOP; -- sol veg obj lod
END LOOP;  -- sol veg obj

---------------------------------------------------------------
-- Create LAYER PLANT_COVER_LOD1-4
---------------------------------------------------------------
FOR r IN 
	SELECT * FROM (VALUES
...
codelist_cols_array	:= ARRAY[['solitary_vegetat_object','class'],['solitary_vegetat_object','function'],['solitary_vegetat_object','usage']];
...
FROM (
	SELECT count(o.id) AS n_features
	FROM 
		',qi_cdb_schema,'.plant_cover AS o
		INNER JOIN ',qi_cdb_schema,'.cityobject AS co ON (o.id = co.id AND o.objectclass_id = ',r.class_id,' ',sql_where,')
	WHERE
		o.',t.lodx_label,'_multi_solid_id IS NOT NULL
	UNION ALL
	SELECT count(o.id) AS n_features
	FROM 
		',qi_cdb_schema,'.plant_cover AS o
...
curr_class := r.class_name;
l_name			:= concat(cdb_schema,'_',r.class_label,'_',t.lodx_label);
--av_name			:= concat('_a_',cdb_schema,'_veg_plant_cover');
gv_name			:= concat('_g_',l_name);
qml_form_name  := 'veg_plant_cover_form.qml';
qml_symb_name  := 'poly_dark_green_symb.qml';
qml_3d_name    := 'poly_dark_green_3d.qml';
trig_f_suffix := 'plant_cover';
qi_l_name  := quote_ident(l_name); ql_l_name := quote_literal(l_name);
qi_gv_name  := quote_ident(gv_name); ql_gv_name := quote_literal(gv_name);
--qi_av_name   := quote_ident(av_name); ql_av_name := quote_literal(av_name);
...
sql_layer := concat(sql_layer, qgis_pkg.generate_sql_matview_header(qi_usr_schema, qi_gv_name),'
	...
	FROM
		',qi_cdb_schema,'.plant_cover AS o
		INNER JOIN ',qi_cdb_schema,'.cityobject AS co ON (o.id = co.id AND o.objectclass_id = ',r.class_id,' ',sql_where,')
		INNER JOIN ',qi_cdb_schema,'.surface_geometry AS sg ON (sg.root_id = o.',t.lodx_label,'_multi_solid_id AND sg.geometry IS NOT NULL)
	WHERE
		o.',t.lodx_label,'_multi_solid_id IS NOT NULL 
	GROUP BY sg.cityobject_id
	UNION
...
FROM
	',qi_usr_schema,'.',qi_gv_name,' AS g 
	INNER JOIN ',qi_cdb_schema,'.cityobject AS co ON (g.co_id = co.id AND co.objectclass_id = ',r.class_id,')
  	INNER JOIN ',qi_cdb_schema,'.plant_cover AS o ON (o.id = co.id AND o.objectclass_id = ',r.class_id,');
COMMENT ON VIEW ',qi_usr_schema,'.',qi_l_name,' IS ''View of ',r.class_name,' ',t.lodx_name,' in schema ',qi_cdb_schema,''';
ALTER TABLE ',qi_usr_schema,'.',qi_l_name,' OWNER TO ',qi_usr_name,';
');
...
EXCEPTION
	WHEN QUERY_CANCELED THEN
		RAISE EXCEPTION 'qgis_pkg.generate_sql_layers_vegetation(): Error QUERY_CANCELED';
	WHEN OTHERS THEN
		RAISE EXCEPTION 'qgis_pkg.generate_sql_layers_vegetation(): %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
