...
-- ***********************************************************************
--
-- This script creates the qgis_pkg schema, where all main functions, the
-- tables used as template and other database, the qgis_pkg_usrgroup role, 
-- and other objects (types, triggers, etc.) will be installed. 
--
-- ***********************************************************************

-- Drops the qgis_pkg schema from a previous installation
...
IF NOT EXISTS(SELECT 1 FROM information_schema.enabled_roles AS i WHERE role_name::varchar = 'qgis_pkg_usrgroup') THEN
	RAISE NOTICE 'Adding role (group) "qgis_pkg_usrgroup"';
	-- Add/create role (user group)
	CREATE ROLE qgis_pkg_usrgroup WITH
	  NOLOGIN
	  NOSUPERUSER
	  INHERIT
	  NOCREATEDB
	  NOCREATEROLE
	  NOREPLICATION;
	COMMENT ON ROLE qgis_pkg_usrgroup IS 'Contains all users that are allowed to use the qgis_pkg';
ELSE
	RAISE NOTICE 'Group "qgis_pkg_usrgroup" already existing';
END IF;
...
CONSTRAINT		extents_template_schema_bbox_unique UNIQUE (cdb_schema, bbox_type)
);
COMMENT ON TABLE qgis_pkg.extents_template IS ''Extents (as bounding box) of data and queries'';

INSERT INTO qgis_pkg.extents_template (cdb_schema, bbox_type, label) VALUES
...
id				bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
cdb_schema		varchar,
layer_type		varchar, -- Value in (VectorLayer, AttribLayer, ...)
feature_type	varchar, -- CityGML package/module name
root_class		varchar, 
class			varchar,
lod				varchar(4), -- value in ('lod0', 'lod1', 'lod2', 'lod3', 'lod4', 'lodx' for no lod)
layer_name		varchar UNIQUE, -- contains the layer names
--v_name			varchar,	-- contains the attribute view names
av_name			varchar, -- contains the attribute view names
--mv_name			varchar,	-- contains the geometry (materialized) view names
gv_name			varchar,
n_features		integer,
creation_date	timestamptz(3),
...
COMMENT ON TABLE qgis_pkg.layer_metadata_template IS 'List of layers in schema qgis_pkg';
...
CREATE INDEX lmeta_av_name_idx    ON qgis_pkg.layer_metadata_template (av_name);
...
--TRUNCATE    qgis_pkg.codelist_template CASCADE RESTART IDENTITY;
...
--TRUNCATE    qgis_pkg.enumeration_value_template CASCADE RESTART IDENTITY;
