...
def fill_database_users_box(cdbMain: CDBToolsMain, usr_names: tuple = None) -> None:
...
def fill_plugin_users_box(cdbMain: CDBToolsMain, usr_names: tuple = None) -> None:
...
def fill_cdb_schemas_box(cdbMain: CDBToolsMain, cdb_schemas_with_priv: list = None) -> None:
...
def setup_post_qgis_pkg_installation(cdbMain: CDBToolsMain) -> None:
    ...
    ts_wf.tabSettings_reset(cdbMain) # This also disables it

    # Reset the user installation GroupBox
    ti_wf.gbxUserInst_reset(cdbMain)
    ...
    db_usr_names: tuple = sql.exec_list_qgis_pkg_non_usrgroup_members(cdbMain)
    ...
    usr_names_plugin: tuple = sql.exec_list_qgis_pkg_usrgroup_members(cdbMain)
    ...
    ti_wf.fill_plugin_users_box(cdbMain, usr_names_plugin)
...
def setup_post_qgis_pkg_uninstallation(cdbMain: CDBToolsMain) -> None:
    """ Function to set up the widgets after:
    ...
    ti_wf.gbxUserInst_reset(cdbMain) # this also disables it
...
def tabInstall_reset(cdbMain: CDBToolsMain) -> None:
    """Function to reset the 'Settings' tab. Resets: gbxInstall and lblInfoText.
    """
    gbxMainInst_reset(cdbMain)
    gbxUserInst_reset(cdbMain)
    gbxConnStatus_reset(cdbMain)


def gbxMainInst_reset(cdbMain: CDBToolsMain) -> None:
...
def gbxUserInst_reset(cdbMain: CDBToolsMain) -> None:
    """Function to reset the 'User Installation Group membership' groupBox
    """
    dlg = cdbMain.admin_dlg

    dlg.gbxUserInst.setDisabled(True)
    gbxGroupMemb_reset(cdbMain)
    gbxUserSchemaInst_reset(cdbMain)
    gbxPriv_reset(cdbMain)


def gbxGroupMemb_reset(cdbMain: CDBToolsMain) -> None:
...
def gbxUserSchemaInst_reset(cdbMain: CDBToolsMain) -> None:
...
def gbxPriv_reset(cdbMain: CDBToolsMain) -> None:
